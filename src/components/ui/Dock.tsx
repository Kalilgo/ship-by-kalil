...
function DockMagnifyItem({
  ...
  return (
    <div className="relative flex flex-col items-center gap-1 pb-0.5">
      <div className="group/dock-tip relative flex flex-col items-center">
        <span
          role="tooltip"
...
function DockSeparator() {
  return (
    <div
      className="mx-1 mb-2 h-10 w-px shrink-0 self-end rounded-full bg-border/70"
      aria-hidden
    />
  );
}
...
function WhatsAppDockIcon({ className = '' }: { className?: string }) {
  return (
    <span
      className={`box-border flex aspect-square h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#25D366] shadow-inner ring-1 ring-black/10 [transform:translateZ(0)] ${className}`}
      aria-hidden
    >
      <svg viewBox="0 0 24 24" className="h-[22px] w-[22px] shrink-0 text-white [transform:none]" fill="currentColor">
...
export function Dock({
  ...
  return (
    <nav
      className="mx-auto w-fit max-w-[min(100vw-1rem,52rem)] rounded-[1.35rem] border border-border/90 bg-surface-2/65 px-2 pb-2 pt-2 shadow-[0_16px_48px_-16px_rgba(0,0,0,0.55)] backdrop-blur-xl sm:px-3"
      aria-label={labels.dockTitle}
    >
      <motion.div
        onMouseMove={(e) => mouseX.set(e.pageX)}
        onMouseLeave={() => mouseX.set(Number.POSITIVE_INFINITY)}
        className="flex items-end justify-center gap-1.5 sm:gap-2.5"
      >
        {settingsMinimized ? (
          <DockMagnifyItem
            mouseX={mouseX}
